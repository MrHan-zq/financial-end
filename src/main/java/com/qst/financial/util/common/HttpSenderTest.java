package com.qst.financial.util.common;

public class HttpSenderTest {
	/*public static void main(String[] args) throws UnsupportedEncodingException {
		String url="http://a242g52637.zicp.vip:37187/KissData?method=";
		url=url+"getProfitOccurrence";
		JSONObject jsonParam = new JSONObject();  
		jsonParam.put("year", 2016);
		jsonParam.put("period", 12);
		url=url+"&json="+URLEncoder.encode(jsonParam.toString(),"UTF-8");
		System.out.println(HttpClentPost.postJson(url, null));
		String url = "https://sms.253.com/msg/";// 应用地址
		String un = "N3947165";// 账号
		String pw = "lwUWIn3MKjc065";// 密码
		String phone = "13058127893";// 手机号码，多个号码使用","分割
		String msg = "【253云通讯】您好，你的验证码是123456";// 短信内容
		String rd = "1";// 是否需要状态报告，需要1，不需要0
		String ex = null;// 扩展码

		
		try {
			String returnString = HttpSender.batchSend( phone, msg, rd, ex);
			System.out.println(returnString);
			// TODO 处理返回值,参见HTTP协议文档
		} catch (Exception e) {
			// TODO 处理异常
			e.printStackTrace();
		}
	}*/
}

